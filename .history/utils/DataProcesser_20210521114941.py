...
class DataProcesser():
    def __init__(self, dataloader):
        ...
        self.data_isdf = isinstance(self.data, pd.core.frame.DataFrame)
        if isinstance(self.data, pd.core.frame.DataFrame):
            print("DataFrame Passed in. Only `.process_pre() can be used to generate df`")
            self.data.df = dataloader
        elif isinstance(self.data, SHLDataLoader):
