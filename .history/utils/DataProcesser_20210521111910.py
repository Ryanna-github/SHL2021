...
class DataProcesser():
    def __init__(self, data_df):
        ...
        if not isinstance(self.data, pd.DataFrame):
            raise TypeError("Wrong Type: {}".format(self.data_df.dtype))

    def process_loc(self):
        # prepare
        self.data['time_dlt'] = self.data['time'].diff().fillna(method = 'bfill')
        self.data['valid_dlt'] = self.data.apply(lambda x: int(x['time_dlt'] <= 10000), axis = 1)
        # utm loc
        self.data['east'] = self.data.apply(lambda x: helper.gps2utm_east(x), axis = 1)
        self.data['north'] = self.data.apply(lambda x: helper.gps2utm_north(x), axis = 1)
        self.data['east_dlt'] = self.data['east'].diff(1)
        self.data['north_dlt'] = self.data['north'].diff(1)
        # speed
        self.data['east_speed'] = self.data.apply(lambda x: x['east_dlt']/x['time_dlt']*1000 if x['valid_dlt'] == 1 else np.nan, axis = 1)
        self.data['north_speed'] = self.data.apply(lambda x: x['north_dlt']/x['time_dlt']*1000 if x['valid_dlt'] == 1 else np.nan, axis = 1)
        self.data['east_speed'] = self.data['east_speed'].apply(lambda x: x if np.abs(x) < 300 else np.nan)
        self.data['north_speed'] = self.data['north_speed'].apply(lambda x: x if np.abs(x) < 300 else np.nan)
        self.data['speed'] = self.data.apply(lambda x: np.sqrt(x['east_speed']**2 + x['north_speed']**2), axis = 1)
        self.data['speed_dif'] = self.data.apply(lambda x: np.abs(x['east_speed'] - x['north_speed']), axis = 1)
        # acc 
        self.data['acc'] = self.data.apply(lambda x: x['speed']/x['time_dlt'] if x['valid_dlt'] == 1 else np.nan, axis = 1)
    ...
    def process_wifi(self):
        ...
        self.data = pd.merge(self.data, tmp_wifi, on = ['time'], how = 'left')
    ...
    def process_cells(self):
        ...
        self.data = pd.merge(self.data, tmp_cells, on = ['time'], how = 'left')
    ...
    def process_gps(self):
        ...
        self.data = pd.merge(self.data, tmp_gps, on = ['time'], how = 'left')
    ...
    def one_hot_transform(self, col_name_list):
        self.self.data_hot = self.self.data.copy()
        # discrete v
        dis_cols = ['cells_ctype_mode']
        for col in dis_cols:
            self.self.data_hot[col].fillna("miss", inplace = True)
            df_hot = helper.get_one_hot(self.self.data_hot, col)
            self.self.data_hot = pd.concat([self.self.data_hot, df_hot]) # ========== 不对，修改！
            self.self.data_hot.drop(col, axis = 1, inplace = True)
