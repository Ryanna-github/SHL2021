...
class DataProcesser():
    ...
    def process_wifi(self):
        ...
        # v2 supplement
        tmp_wifi['wifi_freq_5ratio'] = self.data.wifi_detail[['time', 'wifi_freq']].groupby(['time'], as_index = False).agg(lambda x: x.sum()/len(x))
        t.groupby(['time'], as_index = False).agg(lambda x: x.sum()/len(x))
